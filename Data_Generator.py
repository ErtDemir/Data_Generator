...
def calc_data_len(distance, count_month):  # Calculate the data length based on distance between sequential 2 data and giving number of month
...
def change_append_ec(current_ec, part_of_day, df, lower_bound, current_time, colm, anomaly_time_list, decisive_number):
    ...
    if status == "normal":
        ...
        if current_ec < lower_bound:  # Adding ec some case like Double adding. ex: 1149 -> 1162 -> 1173 but mostly it does one adding process
            ...
            second_row_dict = {colm[0]: second_new_time, colm[1]: second_new_ec, colm[2]: "normal"}
...
for i in range(dataLen):
    ...
    if i % 42500 == 0:  # Period length of the experiment period
        currentEc = 150

# Export the DATA FRAMES as csv file
DataPath = "C:/Users/Ertugrul Demir/Desktop/syntheticData.csv"
...
AnoDatePath = "C:/Users/Ertugrul Demir/Desktop/AnoDate.csv"
anoDateDf = pd.DataFrame(anomalyTimeList)
anoDateDf.to_csv(AnoDatePath, index=False, header=True)
